...
> Auto-generated by Cartographer. Last mapped: 2026-04-12T00:00:00Z
...
**Purpose:** POSIX/GNU CLI argument parsing, populating a `ConfigurationBuilder` from `String[]`. Now includes a fluent `Command` builder, environment variable fallback, and help section grouping.

| Type | Role |
|---|---|
| `CommandLineParser` | Core parser; registers `Option` classes by prefix, parses `String[]` into `ConfigurationBuilder` |
| `CommandLine` | Marker interface + annotation nest; `@Prefix`, `@Prefixes`, `@Description`; inner `Argument` class |
| `Command` | **New** fluent builder: `Command.create("name").argument(...).section(...).option(...).envVar(...).build()` |

**New features:**
- **`Command` builder** — chains `argument`, `helpHandler`, `unknownOption`, `section`, `option`, `envVar`, `envVarResolver`, and `build()`. Defers reflection until `build()`; rethrows `NoSuchMethodException` as `IllegalArgumentException`.
- **Environment variable fallback** — `registerEnvVar(envVarName, prefix)` maps env var names to registered prefixes; env-var-derived args are **prepended** before actual args so explicit CLI args always take precedence. `setEnvironmentVariableResolver(Function<String, Optional<String>>)` defaults to `System::getenv`, overridable for testing.
- **Help section grouping** — `setHelpSection(name)` groups subsequently registered options under a named heading in help output; empty string = unsectioned.

**Gotchas:**
- Env-var args are prepended — explicit CLI args override them (last-write-wins during parsing).
- `=` is globally split before parsing — `--option=foo=bar` becomes three tokens.
- `CAPTURE_UNKNOWN_OPTIONS_AS_ARGUMENTS` rejects hyphen-prefixed args that don't match any prefix.
...
```java
Command command = Command.create("myapp")
    .option("--timeout", Timeout.class, "of", String.class)
    .envVar("MY_TIMEOUT", "--timeout")
    .section("Advanced Options")
    .option("--host", HostName.class, "of", String.class)
    .build();
Configuration config = command.parse(args).build();
